...
header.appendChild(left);header.appendChild(right);codeBlock.prepend(header);const outputEl=codeBlock.querySelector('.webr-output');if(outputEl){outputEl.setAttribute('aria-live','polite');outputEl.setAttribute('aria-atomic','false');outputEl.setAttribute('tabindex','-1');}});const allEditors=document.querySelectorAll('.webr-editor');allEditors.forEach((el,i)=>{_editorIndexMap.set(el,i);el.setAttribute('tabindex','0');el.setAttribute('role','textbox');el.setAttribute('aria-label','R code. Click or press Enter to edit.');});function _textOffsetInside(root,node,offset){if(!root.contains(node))return null;if(node.nodeType!==Node.TEXT_NODE){let total=0;const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);let t;while((t=walker.nextNode())){if(node.contains(t)){const childIndex=Array.prototype.indexOf.call(node.childNodes,t);if(childIndex>=0&&childIndex<offset){total+=t.nodeValue.length;continue;}
return total;}
total+=t.nodeValue.length;}
return total;}
let total=0;const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);let t;while((t=walker.nextNode())){if(t===node)return total+offset;total+=t.nodeValue.length;}
return null;}
function _caretOffsetFromPoint(el,clientX,clientY){if(clientX==null||clientY==null)return null;try{if(document.caretPositionFromPoint){const pos=document.caretPositionFromPoint(clientX,clientY);if(!pos||!pos.offsetNode)return null;return _textOffsetInside(el,pos.offsetNode,pos.offset);}
if(document.caretRangeFromPoint){const r=document.caretRangeFromPoint(clientX,clientY);if(!r)return null;return _textOffsetInside(el,r.startContainer,r.startOffset);}}catch(_){}
return null;}
function hydrateEditor(el,clientX,clientY){if(el.dataset.webrHydrated){const existing=el.querySelector('.webr-editor-input');if(existing)existing.focus();return;}
const code=el.textContent;const caret=_caretOffsetFromPoint(el,clientX,clientY);const ta=document.createElement('textarea');ta.className='webr-editor-input';ta.spellcheck=false;ta.setAttribute('autocapitalize','off');ta.setAttribute('autocorrect','off');ta.setAttribute('autocomplete','off');ta.setAttribute('aria-label','Editable R code. Press Ctrl+Enter to run.');ta.rows=Math.max(code.split('\n').length,3);ta.value=code;el.innerHTML='';el.appendChild(ta);el.dataset.webrHydrated='1';el.classList.add('webr-editor-hydrated');const idx=_editorIndexMap.get(el);editors[idx]={textarea:ta,originalCode:code,el};ta.focus();if(typeof caret==='number'&&caret>=0&&caret<=code.length){ta.selectionStart=ta.selectionEnd=caret;}}
document.addEventListener('click',function(e){const el=e.target&&e.target.closest?e.target.closest('.webr-editor'):null;if(!el||el.dataset.webrHydrated)return;hydrateEditor(el,e.clientX,e.clientY);},true);document.addEventListener('focusin',function(e){const el=e.target&&e.target.closest?e.target.closest('.webr-editor'):null;if(!el||el.dataset.webrHydrated)return;if(e.target.tagName==='TEXTAREA')return;hydrateEditor(el,null,null);},true);document.addEventListener('keydown',function(e){if(e.key!=='Enter'&&e.key!==' ')return;const el=e.target&&e.target.closest?e.target.closest('.webr-editor'):null;if(!el||el.dataset.webrHydrated)return;if(e.target.tagName==='TEXTAREA')return;e.preventDefault();hydrateEditor(el,null,null);},true);document.addEventListener('keydown',function(e){const ta=e.target;if(!ta||!ta.classList||!ta.classList.contains('webr-editor-input'))return;if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();const container=ta.closest('.webr-container');const btn=container&&container.querySelector('.webr-run-btn');if(btn&&!btn.disabled)btn.click();return;}
if(e.key==='Tab'&&!e.shiftKey){e.preventDefault();const s=ta.selectionStart;const en=ta.selectionEnd;ta.setRangeText('  ',s,en,'end');return;}
if(e.key==='Escape'){ta.blur();}});const installedPkgs=new Set(['base','stats','graphics','grDevices','utils','datasets','methods']);async function ensurePackages(code){const libRegex=/(?:library|require)\s*\(\s*["']?([a-zA-Z0-9.]+)["']?\s*\)/g;const needed=[];let match;while((match=libRegex.exec(code))!==null){const pkg=match[1];if(!installedPkgs.has(pkg))needed.push(pkg);}
...
if(!shelter)return;const container=btn.closest('.webr-container');const editorEl=container.querySelector('.webr-editor');const outputEl=container.querySelector('.webr-output');const plotEl=container.querySelector('.webr-plot-output');const idx=_editorIndexMap.get(editorEl);const editorEntry=editors[idx];let code=(editorEntry&&editorEntry.textarea)?editorEntry.textarea.value:editorEl.textContent;btn.disabled=true;setRunBtnText(btn,'Installing packages...');btn.classList.add('running');outputEl.textContent='';outputEl.classList.remove('has-content','has-error','has-message');outputEl.classList.add('is-loading');if(plotEl){plotEl.innerHTML='';plotEl.classList.remove('has-content');}
...
btn.disabled=false;setRunBtnText(btn,'\u25b6 Run');btn.classList.remove('running');};window.resetWebR=async function(btn){const container=btn.closest('.webr-container');const editorEl=container.querySelector('.webr-editor');const outputEl=container.querySelector('.webr-output');const plotEl=container.querySelector('.webr-plot-output');const idx=_editorIndexMap.get(editorEl);const entry=editors[idx];if(entry&&entry.textarea){entry.textarea.value=entry.originalCode;entry.textarea.rows=Math.max(entry.originalCode.split('\n').length,3);}
