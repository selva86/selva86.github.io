...
return null;}
function hydrateEditor(el,clientX,clientY){if(el.dataset.webrHydrated){const existing=el.querySelector('.webr-editor-input');if(!existing)return;existing.focus();const idx2=_editorIndexMap.get(el);const entry2=editors[idx2];if(entry2&&entry2.jar&&typeof clientX==='number'&&typeof clientY==='number'){const off=_caretOffsetFromPoint(existing,clientX,clientY);if(typeof off==='number')entry2.jar.restore({start:off,end:off});}
return;}
if(!window.CodeJar||!window.Prism){return;}
const code=el.textContent;const caret=_caretOffsetFromPoint(el,clientX,clientY);const inputEl=document.createElement('div');inputEl.className='webr-editor-input';inputEl.setAttribute('role','textbox');inputEl.setAttribute('aria-multiline','true');inputEl.setAttribute('aria-label','Editable R code. Press Ctrl+Enter to run.');inputEl.setAttribute('spellcheck','false');inputEl.setAttribute('autocapitalize','off');inputEl.setAttribute('autocorrect','off');inputEl.setAttribute('autocomplete','off');el.innerHTML='';el.appendChild(inputEl);el.dataset.webrHydrated='1';el.classList.add('webr-editor-hydrated');const jar=window.CodeJar(inputEl,_prismHighlight,{tab:'  ',indentOn:/[({\[]$/,catchTab:true,preserveIdent:true,addClosing:false,history:true,});jar.updateCode(code);const idx=_editorIndexMap.get(el);editors[idx]={jar:jar,inputEl:inputEl,originalCode:code,el};inputEl.focus();if(typeof caret==='number'&&caret>=0&&caret<=code.length){try{jar.restore({start:caret,end:caret});}catch(_){}}}
