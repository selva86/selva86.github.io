...
- **Step arc** - the ordered steps, each naming the exact step TITLE (the H2 it will carry; the cover's title must equal the catalog title), the ONE new idea it adds, **the visual that carries it** (widget / diagram / image, chosen from `_build/lesson-visual-catalog.md`) or an explicit `prose-only (why)`, the CODE PLAN (for each planned ```r block: the dataset or package and the one thing the block demonstrates), its check, and a BRIDGE line (`coming from <prior step's idea> -> leading to <next step's idea>`) plus where the running example stands. MEASURE: one-new-idea-per-step; every concept used appears earlier; every step states a real bridge; **every step that introduces something visualizable (a structure, process, distribution, boundary, sample, or relationship) names a visual, never prose alone** - a blank or unjustified `prose-only` on a visualizable concept fails plan review.
- **Narrative spine** - one paragraph at the top of the plan telling the lesson's story start to finish; the step arc must follow it. MEASURE: paragraph present; the judge confirms the steps follow it.
- **Plan approval** - the plan is written by a dedicated planning session (`--plan-only`), reviewed by a fresh `/check-lesson-plan` session that fixes flow directly and flips `status: draft` to `status: approved`, and only then built (`--build`). The builder may ADD depth beyond the approved plan (R13) but may never reorder or cut it. MEASURE: the plan carries `status: approved` before any prose exists.
