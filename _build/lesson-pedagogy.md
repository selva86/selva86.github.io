...
- **Step arc** - the ordered steps, each naming the ONE new idea it adds + its visual/widget/check + why. MEASURE: one-new-idea-per-step; every concept used appears earlier.
...
- **R6 Interactives earn their place.** A widget appears only at a "feel-it" moment (an abstract idea made manipulable), computes REAL results (no faked numbers/curves), reinforces the concept just taught, and is drawn from the widget library (a genuinely novel widget is flagged for hand-build, never faked). MEASURE: each widget maps to a concept; numbers are real.
...
| `lesson_quality_check.py` (deterministic) | cover visual (R1), >=1 try-it + >=1 quiz (R5), 3-5 references that resolve (R9), every `::widget` type exists, every gated step in the grading manifest, `mathjax:true` when formulas are present |
| LLM-judge (rubric, fresh agent) | from-scratch (R2), depth ladder (R3), continuity (R4), distractor quality (R5), gap critique (R8) |
