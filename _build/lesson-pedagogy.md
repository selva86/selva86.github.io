...
- **R13 One sitting (length).** A lesson is ONE coherent slice, sized to finish in a single ~8 to 10 minute sitting: target **7 to 12 steps**, ceiling **12**. Past 12 it is almost always two lessons bundled - split into a multi-lesson course AT A NATURAL CONCEPTUAL SEAM (never mid-arc), each part a complete cover -> teach -> check -> complete with its own takeaway. Treat 12 as a near-hard limit: only a genuinely indivisible single arc may exceed it, and never past ~14; a loose lesson covering two ideas should split even under 12. Plan the split UP FRONT in the arc, do not chop a finished lesson. MEASURE: gate WARNS at > 12 steps; judge confirms each lesson is one coherent slice with its own payoff AND that anything over 12 truly could not be split.
