...
- **R13 One sitting (length).** A lesson is ONE coherent slice, sized to finish in a single ~8 to 10 minute sitting: target **7 to 12 steps**, soft ceiling **~15**. Past that it is almost always two lessons bundled - split into a multi-lesson course AT A NATURAL CONCEPTUAL SEAM (never mid-arc), each part a complete cover -> teach -> check -> complete with its own takeaway. The count is a signal, not a law: a tight 14-step single arc may stay; a loose 12-step covering two ideas should split. Plan the split UP FRONT in the arc, do not chop a finished lesson. MEASURE: gate WARNS at > 15 steps; judge confirms each lesson is one coherent slice with its own payoff.
