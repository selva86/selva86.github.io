...
| `isolation-forest` | cluster-point vs outlier toggle -> live random axis-aligned splits isolating the point; shows path length + anomaly score (short path = anomaly); emits a from-scratch isolation forest in base R | §20 isolation forest / extended isolation forest |
| `lof-density` | k = 5/8/12 toggle -> scatter coloured + sized by Local Outlier Factor; a locally-sparse point lights up (LOF >> 1); emits a from-scratch LOF via dist + kNN density ratios | §20 local outlier factor, density-based detection |
| `autoencoder-recon` | normal-point vs anomaly toggle -> points hugging a bottleneck line (the learned manifold) with a red residual = reconstruction error; anomaly sits off-manifold with a large error; emits PCA reconstruction error (linear autoencoder) in base R | §20 autoencoders for anomaly detection |
| `conformal-bands` | target 80/90/95% toggle -> a regression band y-hat +/- q-hat (half-width from the calibration-residual quantile) widens with the target while empirical coverage on test points tracks at/above target; red = points outside; emits split conformal in base R | §21 conformal prediction, guaranteed-coverage intervals |
