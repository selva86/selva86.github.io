...
WEBR_HEAD_BLOCK = """
    ...
    <style>
      ...
      .webr-header-dots { display: flex; gap: 5px; }
      .webr-header-dots span {
        width: 10px; height: 10px; border-radius: 50%; display: inline-block;
      }
      .webr-header-dots span:nth-child(1) { background: #ff5f57; }
      .webr-header-dots span:nth-child(2) { background: #febc2e; }
      .webr-header-dots span:nth-child(3) { background: #28c840; }
      .webr-header-title {
        color: #94a3b8; font-size: 11px; font-weight: 500; letter-spacing: 0.3px;
      }
      .webr-header-badge {
        background: #334155; color: #94a3b8; padding: 1px 8px;
        border-radius: 4px; font-size: 10px; font-weight: 600; letter-spacing: 0.5px;
      }
...
WEBR_BODY_BLOCK = """
  ...
  <script type="module">
    ...
    // Add terminal header bar to each code block
    document.querySelectorAll('.webr-container').forEach(container => {
      ...
      header.innerHTML = '<div class="webr-header-dots"><span></span><span></span><span></span></div>'
        + '<span class="webr-header-title">R Console</span>'
    ...
    window.runWebR = async function(btn) {
      ...
      if (!webRReady) {
        if (webRLoading) {
          ...
          if (!webRReady) { btn.disabled = false; btn.textContent = '\\u25b6 Run'; btn.classList.remove('running'); return; }
        } else {
          btn.disabled = true;
          btn.textContent = 'Loading R...';
          btn.classList.add('running');
          const ok = await loadAndInitWebR();
          if (!ok) { btn.disabled = false; btn.textContent = '\\u25b6 Run'; btn.classList.remove('running'); return; }
        }
      }
      if (!shelter) return;
