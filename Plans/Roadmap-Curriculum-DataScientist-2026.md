...
1. **From business question to a validated model** - CRISP-DM, the modeling mindset, and why a simple baseline comes first `(R)`
2. **The bias-variance tradeoff** - signal, noise and generalization, with a note on double descent `(R)`
3. **Train, validation, test, and data leakage** - the discipline and the leakage taxonomy: target, temporal, train-serving `(R)`
...
1. **Survival data and Kaplan-Meier** - censoring, the survival and hazard functions, and the log-rank test `(R)`
