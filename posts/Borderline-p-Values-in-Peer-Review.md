---
title: "Borderline p-Values in Peer Review"
slug: Borderline-p-Values-in-Peer-Review
description: "A reviewer says your p = 0.049 is being treated as a definitive finding. How to check whether a borderline p-value actually holds up in R, and word the reply."
keywords: "borderline p-value, p = 0.049 significant, marginal p-value reviewer, is p just under 0.05 real, borderline significance peer review, p-value just below 0.05, barely significant result"
mathjax: false
webr: true
date: 2026-08-07
curriculum_id: null
...
handbook_part: 9
handbook_chapter: 45
auto_link_terms: borderline p-value|p = 0.049|marginal significance|just under 0.05|borderline significance|barely significant result|p-value just below 0.05
auto_link_case_sensitive: false
difficulty: Intermediate
---


<p class="lead">A borderline p-value is one that falls just inside the 0.05 cut-off, close enough that a single data point or a defensible change of test could push it back out. Reviewers raise it when a result at p = 0.047 is written up with the same confidence as one at p = 0.001. The answer is not to argue the threshold but to check how stable the result is, report the effect size and interval beside the p-value, and match your wording to the evidence.</p>
...
The second check is whether the verdict depends on the exact data and the exact test. Refitting the comparison while dropping each treatment plant in turn, and rerunning it as a rank-based [Wilcoxon test](/Mann-Whitney-U-Test-in-R.html) that makes no normality assumption, shows how far the p-value travels.

```r
round(c(reported = t.test(trt2, ctrl)$p.value,
        wilcoxon = wilcox.test(trt2, ctrl)$p.value,
        drop_one = max(sapply(seq_along(trt2),
                       function(i) t.test(trt2[-i], ctrl)$p.value))), 3)
#> reported wilcoxon drop_one 
#>    0.048    0.063    0.085 
```
...
```r
ex_low <- subset(ToothGrowth, dose == 0.5)
t.test(len ~ supp, data = ex_low)
```
