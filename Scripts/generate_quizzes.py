...
TIDYR_BANK = [
    ...
    {'id':'tr_tidy_def', 'type':'mcq', 'diff':1, 'topic':'tidy data',
     ...
     'why':'Tidy data rules: (1) each variable forms a column, (2) each observation forms a row, (3) each type of observational unit forms a table. This shape is what dplyr and ggplot2 expect.'},
    {'id':'tr_pivot_longer_basic', 'type':'mcq', 'diff':1, 'topic':'reshape',
     ...
     'why':'<code>pivot_longer()</code> stacks selected columns into name/value pairs (long format). <code>gather()</code> is the deprecated tidyr 0.x ancestor, still functional but not recommended for new code.'},
    {'id':'tr_separate_basic', 'type':'mcq', 'diff':1, 'topic':'columns',
     'q':'What does <code>separate(col, into = c("a", "b"), sep = "_")</code> do?',
     'opts':[
        {'t':'Splits one column into multiple columns at the delimiter','correct':True},
        {'t':'Drops the column'},
        {'t':'Computes a row-wise mean'},
        {'t':'Joins two columns into one'},
     ],
     'hints':['"Separate" splits, "unite" joins.'],
     'why':'<code>separate()</code> splits a single character column into multiple columns at the delimiter (or by regex/positions). The inverse is <code>unite()</code>. In tidyr 1.3+ prefer <code>separate_wider_delim()</code> for clearer errors on too-few/too-many pieces.'},
    {'id':'tr_drop_na_basic', 'type':'mcq', 'diff':1, 'topic':'missing',
     ...
     'hints':['drop_na drops rows; select(-col) drops columns.'],
     'why':'<code>drop_na(col)</code> removes rows where col is NA. <code>drop_na()</code> with no args drops rows where ANY column is NA (same as <code>na.omit()</code>).'},

    # Medium MCQ (4)
    {'id':'tr_pivot_longer_cols', 'type':'mcq', 'diff':2, 'topic':'reshape',
     ...
     'why':'<code>cols</code> uses tidy-select syntax. <code>-id</code> means "every column except id". id is preserved as the row identifier; everything else stacks into key/val pairs.'},
    {'id':'tr_pivot_wider_args', 'type':'mcq', 'diff':2, 'topic':'reshape',
     ...
     'opts':[
        ...
        {'t':'A single column into multiple columns by splitting'},
        {'t':'A data frame into a list'},
     ],
     'hints':['names_from supplies new column NAMES; values_from supplies the cell VALUES.'],
     'why':'<code>pivot_wider()</code> spreads a long table to wide: unique key values become column names, val values fill the cells. The inverse of <code>pivot_longer()</code>. If a (id, key) pair has duplicates, you must supply <code>values_fn</code> to aggregate.'},
    {'id':'tr_unnest_basic', 'type':'mcq', 'diff':2, 'topic':'nested',
     'q':'You have a tibble with a list-column called <code>data</code>, where each cell holds a tibble. What does <code>unnest(data)</code> do?',
     'opts':[
        {'t':'Flattens the list-column so each inner row becomes its own row in the outer tibble','correct':True},
        {'t':'Wraps each value in a list'},
        {'t':'Pivots the list-column to wide format'},
        {'t':'Drops the list-column'},
     ],
     'hints':['List-columns can hold any per-row payload. unnest expands them into long form.'],
     'why':'<code>unnest()</code> expands list-columns into long form: a row whose list-cell contains a 3-row tibble becomes 3 rows in the output. The inverse is <code>nest()</code>. For atomic vectors use <code>unnest_longer()</code> or <code>unnest_wider()</code>.'},
    {'id':'tr_complete_grid', 'type':'mcq', 'diff':2, 'topic':'completing',
     'q':'<code>complete(year, group)</code> on a tibble does what?',
     'opts':[
        {'t':'Drops rows with NAs in year or group'},
        {'t':'Adds rows so every combination of year and group appears, filling new rows with NA','correct':True},
        {'t':'Removes duplicate rows'},
        {'t':'Fills NAs with the previous non-NA value'},
     ],
     'hints':['complete() ensures every combination of the named variables exists.','For "carry previous value" use fill(), not complete().'],
     'why':'<code>complete()</code> expands the data so every combination of the listed variables is present, filling new rows with NA (or with values via <code>fill =</code>). Useful before time-series joins or to avoid implicit zeros becoming missing.'},

    # Hard MCQ (3)
    {'id':'tr_names_sep_vs_pattern', 'type':'mcq', 'diff':3, 'topic':'reshape',
     'q':'You have wide columns <code>income_2020, income_2021, expense_2020, expense_2021</code>. Which call splits each name on the underscore into TWO new ID columns (kind, year) in one step?',
     'opts':[
        {'t':'<code>pivot_longer(cols = -id, names_to = c("kind", "year"), names_sep = "_")</code>','correct':True},
        {'t':'<code>pivot_longer(cols = -id, names_to = "kind_year")</code> then separate()'},
        {'t':'Two separate pivot_longer() calls then bind_rows()'},
        {'t':'<code>separate_rows()</code> then pivot_longer()'},
     ],
     'hints':['names_to accepts a vector when paired with names_sep or names_pattern.','names_sep splits on a literal delimiter; names_pattern uses a regex with capture groups.'],
     'why':'<code>names_to = c("kind", "year")</code> with <code>names_sep = "_"</code> splits each column name at the underscore into two new ID columns inside one pivot_longer call. Use <code>names_pattern = "(.*)_(.*)"</code> (regex with capture groups) when the split rule is more complex than a single literal.'},
    {'id':'tr_names_pattern_vs_sep', 'type':'mcq', 'diff':3, 'topic':'reshape',
     'q':'For wide columns like <code>x10y20</code>, <code>x15y25</code> where you need to extract digit groups, which pivot_longer argument do you need?',
     'opts':[
        {'t':'<code>names_sep = ""</code>'},
        {'t':'<code>names_pattern = "x(\\\\d+)y(\\\\d+)"</code>','correct':True},
        {'t':'<code>names_to = "name"</code> with no other arg'},
        {'t':'<code>cols_vary = "fastest"</code>'},
     ],
     'hints':['names_sep takes a delimiter character or position; it cannot capture submatches.','names_pattern takes a regex with capture groups, one per element of names_to.'],
     'why':'<code>names_sep</code> only handles delimiters or fixed positions. When the rule is "extract these digit groups out of this mixed pattern" you need <code>names_pattern</code>, a regex whose capture groups align with names_to. This is the most common reason a names_sep approach fails.'},
    {'id':'tr_separate_wider_family', 'type':'mcq', 'diff':3, 'topic':'columns',
     'q':'In tidyr 1.3+, <code>separate()</code> was superseded by which family?',
     'opts':[
        {'t':'<code>extract()</code> only'},
        {'t':'<code>separate_wider_delim()</code>, <code>separate_wider_position()</code>, <code>separate_wider_regex()</code> (and matching separate_longer_*)','correct':True},
        {'t':'<code>split_columns()</code>'},
        {'t':'<code>pivot_longer()</code>'},
     ],
     'hints':['tidyr 1.3 introduced separate_wider_delim / _position / _regex (and separate_longer_*).','Old separate() still works but the new family has clearer error messages.'],
     'why':'tidyr 1.3 split <code>separate()</code> into <code>separate_wider_delim</code> (by delimiter), <code>_position</code> (by character index), and <code>_regex</code> (by regex with named pieces), plus the long-form variants. The new family forces explicit handling of too-few and too-many pieces via <code>too_few =</code> and <code>too_many =</code>, eliminating silent truncation.'},
    ...
    {'id':'tr_code_long', 'type':'code', 'diff':1, 'topic':'reshape',
     ...
     'tests':'df <- data.frame(id = 1:2, q1 = c(5, 3), q2 = c(4, 2), q3 = c(5, 5))\nr <- to_long(df)\nstopifnot(nrow(r) == 6)\nstopifnot(all(c("id", "question", "score") %in% names(r)))\nstopifnot(identical(sort(unique(as.character(r$question))), c("q1", "q2", "q3")))\nstopifnot(setequal(r$score, c(2, 3, 4, 5)))',
     'hints':['pivot_longer(df, cols = -id, names_to = "question", values_to = "score").'],
     'why':'<code>pivot_longer(df, cols = -id, names_to = "question", values_to = "score")</code>. Excluding id preserves it as the row identifier; q1/q2/q3 stack into name/value pairs.'},
    {'id':'tr_code_wide', 'type':'code', 'diff':1, 'topic':'reshape',
     ...
     'hints':['pivot_wider(df, names_from = key, values_from = value).'],
     'why':'<code>pivot_wider(df, names_from = key, values_from = value)</code>. Each unique key value becomes a new column; (id, key) must be unique or you must supply <code>values_fn</code>.'},

    # Medium code (2)
    {'id':'tr_code_separate', 'type':'code', 'diff':2, 'topic':'columns',
     'prompt':'Write <code>split_full_name(df)</code> taking a tibble with column <code>full_name</code> (e.g. "Alex Morgan"). Return the tibble with two new columns <code>first</code> and <code>last</code> from splitting on the space, and drop the original <code>full_name</code> column.',
     'fnName':'split_full_name',
     'starter':'library(tidyr)\n\nsplit_full_name <- function(df) {\n  # Your code here\n  \n}',
     'tests':'df <- data.frame(full_name = c("Alex Morgan", "Bri Cole"), age = c(30, 25), stringsAsFactors = FALSE)\nr <- split_full_name(df)\nstopifnot(all(c("first", "last") %in% names(r)))\nstopifnot(!"full_name" %in% names(r))\nstopifnot(r$first[1] == "Alex")\nstopifnot(r$last[2] == "Cole")\nstopifnot(r$age[1] == 30)',
     'hints':['separate(df, full_name, into = c("first", "last"), sep = " ") works.','Or use separate_wider_delim(df, full_name, delim = " ", names = c("first", "last")).','By default both drop the source column.'],
     'why':'<code>separate(df, full_name, into = c("first", "last"), sep = " ")</code> (or the modern <code>separate_wider_delim()</code>). Both drop the source column by default; pass <code>remove = FALSE</code> to keep it.'},
    {'id':'tr_code_dropna_any', 'type':'code', 'diff':2, 'topic':'missing',
     ...
     'tests':'df <- data.frame(a = c(1, NA, 3), b = c("x", "y", NA), c = c(10, 20, 30), stringsAsFactors = FALSE)\nr <- complete_responses(df)\nstopifnot(nrow(r) == 1)\nstopifnot(r$a == 1)\nstopifnot(r$b == "x")',
     'hints':['drop_na() with no column arguments drops rows where ANY column is NA.','That is the default behavior.'],
     'why':'<code>drop_na(df)</code> with no column args drops any row with at least one NA in any column. Equivalent to base R <code>na.omit(df)</code>, but returns a tibble and integrates with pipes.'},

    # Hard code (2)
    {'id':'tr_code_separate_wider', 'type':'code', 'diff':3, 'topic':'columns',
     'prompt':'Write <code>split_code(df)</code> that takes a tibble with column <code>code</code> containing strings like "A-2020-Q1". Use <code>separate_wider_delim()</code> to split into three new columns <code>region</code>, <code>year</code>, <code>quarter</code> on the "-" delimiter. Drop the original column.',
     'fnName':'split_code',
     'starter':'library(tidyr)\n\nsplit_code <- function(df) {\n  # Your code here\n  \n}',
     'tests':'df <- data.frame(code = c("A-2020-Q1", "B-2021-Q4", "C-2022-Q2"), val = c(10, 20, 30), stringsAsFactors = FALSE)\nr <- split_code(df)\nstopifnot(all(c("region", "year", "quarter") %in% names(r)))\nstopifnot(!"code" %in% names(r))\nstopifnot(identical(as.character(r$region), c("A", "B", "C")))\nstopifnot(identical(as.character(r$year), c("2020", "2021", "2022")))\nstopifnot(identical(as.character(r$quarter), c("Q1", "Q4", "Q2")))\nstopifnot(identical(r$val, c(10, 20, 30)))',
     'hints':['separate_wider_delim(df, code, delim = "-", names = c("region", "year", "quarter")).','By default the source column is removed (cols_remove = TRUE).'],
     'why':'<code>separate_wider_delim(df, code, delim = "-", names = c("region", "year", "quarter"))</code>. The wider_* family (tidyr 1.3+) gives clear error messages when a row has too few or too many pieces, controlled via <code>too_few =</code> and <code>too_many =</code>.'},
    {'id':'tr_code_complete_fill', 'type':'code', 'diff':3, 'topic':'completing',
     'prompt':'Write <code>fill_gaps(df)</code> that takes a tibble with columns <code>year, group, value</code>. Ensure every combination of year x group is present (filling missing rows with NA in value), and return the result sorted by group then year. Use tidyr <code>complete()</code> and dplyr <code>arrange()</code>.',
     'fnName':'fill_gaps',
     'starter':'library(tidyr)\nlibrary(dplyr)\n\nfill_gaps <- function(df) {\n  # Your code here\n  \n}',
     'tests':'df <- data.frame(year = c(2020, 2021, 2020, 2022), group = c("A","A","B","B"), value = c(1, 2, 3, 4), stringsAsFactors = FALSE)\nr <- fill_gaps(df)\nstopifnot(nrow(r) == 6)\nstopifnot(all(c("year", "group", "value") %in% names(r)))\nstopifnot(any(is.na(r$value)))\nstopifnot(identical(as.character(r$group), c("A","A","A","B","B","B")))\nstopifnot(identical(r$year, c(2020, 2021, 2022, 2020, 2021, 2022)))',
     'hints':['complete(year, group) adds the missing combinations and fills value with NA by default.','arrange(group, year) sorts as required.'],
     'why':'<code>df %>% complete(year, group) %>% arrange(group, year)</code>. complete() builds the full year x group grid and inserts NA in value for the new rows; arrange() then enforces the requested ordering.'},
]
...
SHINY_BANK = [
    ...
    {'id':'sh_two_parts', 'type':'mcq', 'diff':1, 'topic':'architecture',
     ...
     'why':'A Shiny app has a UI definition (HTML + Shiny widgets declaring inputs and outputs) and a server function (the reactive logic mapping inputs to outputs). <code>shinyApp(ui, server)</code> binds the two halves.'},
    {'id':'sh_input_read', 'type':'mcq', 'diff':1, 'topic':'reactivity',
     ...
     'why':'<code>input$x</code> reads the current value of the input element with inputId = "x". It is a reactive value: any reactive context that reads it will re-execute when it changes. Treat input as read-only inside server; never assign to <code>input$x</code>.'},
    {'id':'sh_render_text', 'type':'mcq', 'diff':1, 'topic':'rendering',
     'q':'Which renderer produces text output for a Shiny app, paired with <code>textOutput()</code>?',
     ...
     'why':'<code>renderText()</code> returns a single character string that displays inline. <code>renderPrint()</code> captures the full print() output (multi-line, with the same formatting you would see at the console) and pairs with <code>verbatimTextOutput()</code>.'},
    {'id':'sh_run_app', 'type':'mcq', 'diff':1, 'topic':'running',
     ...
     'hints':['shinyApp() wraps ui + server into an app object; runApp() runs from a file or directory.'],
     'why':'<code>shinyApp(ui, server)</code> creates an app object. Printing it (or returning it from app.R) launches the local server. <code>runApp("path")</code> runs an app stored as app.R or as a folder with ui.R and server.R.'},

    # Medium MCQ (4)
    {'id':'sh_reactive_basic', 'type':'mcq', 'diff':2, 'topic':'reactivity',
     'q':'What does <code>reactive()</code> do?',
     'opts':[
        {'t':'Schedules a one-time computation'},
        {'t':'Wraps an expression in a cached reactive expression that recomputes lazily when its dependencies change','correct':True},
        {'t':'Disables a Shiny widget'},
        {'t':'Renders HTML output'},
     ],
     'hints':['reactive() returns a function-like reactive expression; call it as v() to get its current value.'],
     'why':'<code>reactive()</code> wraps an expression. Calling the wrapped name as <code>v()</code> returns the cached current value; the value is recomputed lazily (on first read after an upstream change). Use it to share a computed value across multiple outputs without re-running the calculation each time.'},
    {'id':'sh_isolate', 'type':'mcq', 'diff':2, 'topic':'reactivity',
     'q':'What does <code>isolate()</code> do inside a reactive context?',
     'opts':[
        {'t':'Reads a reactive value WITHOUT taking a reactive dependency on it','correct':True},
        {'t':'Stops the app'},
        {'t':'Throttles updates'},
        {'t':'Caches a value to disk'},
     ],
     'hints':['Sometimes you want to read input but NOT re-run when it changes (e.g., trigger only on a button click).'],
     'why':'<code>isolate(input$x)</code> reads x without subscribing to its changes: the surrounding reactive will not re-execute when x updates. Classic pairing: <code>observeEvent(input$go, { isolate(do_something(input$x)) })</code> so the work runs only on click but uses the current x.'},
    {'id':'sh_observe_event_basic', 'type':'mcq', 'diff':2, 'topic':'reactivity',
     'q':'Use <code>observeEvent(input$go, { ... })</code> when you want to:',
     'opts':[
        {'t':'Continuously sync to every input change'},
        {'t':'Run an action ONLY when a specific event (such as a button click) fires','correct':True},
        {'t':'Disable an input'},
        {'t':'Filter a data frame inside a reactive expression'},
     ],
     'hints':['observeEvent fires when its eventExpr changes; other reactive values inside the block are isolated by default.'],
     'why':'<code>observeEvent(input$go, { ... })</code> runs the block only when input$go changes (typically on a button click). Other reactive values referenced inside the body are isolated by default, so the action does not re-fire when they update.'},
    {'id':'sh_layout_fluid', 'type':'mcq', 'diff':2, 'topic':'ui',
     'q':'<code>fluidPage()</code> versus <code>fixedPage()</code>:',
     'opts':[
        {'t':'They are identical'},
        {'t':'fluidPage uses a responsive 12-column grid; fixedPage uses a fixed-width Bootstrap container','correct':True},
     ...
     'why':'<code>fluidPage()</code> renders a responsive Bootstrap container that fills the viewport width. <code>fixedPage()</code> renders a fixed-width container that does not stretch. fluidPage is the modern default for dashboards.'},

    # Hard MCQ (3)
    {'id':'sh_reactive_vs_reactiveval', 'type':'mcq', 'diff':3, 'topic':'reactivity',
     'q':'What is the key difference between <code>reactive()</code> and <code>reactiveVal()</code>?',
     'opts':[
        {'t':'They are aliases for the same thing'},
        {'t':'reactive() wraps a recompute-on-demand expression; reactiveVal() is a mutable single-value reactive store you can set imperatively','correct':True},
        {'t':'reactive() works only in the UI; reactiveVal() works only in the server'},
        {'t':'reactiveVal() caches to disk; reactive() does not'},
     ],
     'hints':['reactive(): expression-based, you cannot set its value.','reactiveVal(): a getter/setter pair, val() reads, val(x) writes.'],
     'why':'<code>reactive(expr)</code> defines a derived value: you read it as <code>v()</code>, never assign. <code>reactiveVal(init)</code> defines a mutable reactive cell: <code>v()</code> reads, <code>v(new)</code> writes. Use reactive() for computed pipelines, reactiveVal() inside observers when you need to remember state (e.g., a click counter).'},
    {'id':'sh_observe_vs_observe_event', 'type':'mcq', 'diff':3, 'topic':'reactivity',
     'q':'What is the difference between <code>observe()</code> and <code>observeEvent()</code>?',
     'opts':[
        {'t':'They are identical'},
        {'t':'observe() re-runs whenever ANY reactive value it reads changes; observeEvent() runs only when its eventExpr changes (other reads are isolated)','correct':True},
        {'t':'observe() runs on the client; observeEvent() runs on the server'},
        {'t':'observe() returns a value; observeEvent() does not'},
     ],
     'hints':['observe() subscribes to everything; observeEvent() subscribes only to its trigger.','Reaching for observe() and then wrapping reads in isolate() is the long-hand of observeEvent().'],
     'why':'<code>observe({ ... })</code> takes a dependency on every reactive value it reads inside the block. <code>observeEvent(trigger, { ... })</code> only fires when <code>trigger</code> changes; other reactive reads inside the handler are isolated automatically. Use observeEvent for buttons and discrete events; reserve observe for "always stay in sync" reactions.'},
    {'id':'sh_event_reactive', 'type':'mcq', 'diff':3, 'topic':'reactivity',
     'q':'<code>eventReactive(input$go, { expensive_calc() })</code> differs from <code>reactive({ expensive_calc() })</code> in that:',
     'opts':[
        {'t':'It is a UI helper, not a server one'},
        {'t':'It recomputes ONLY when input$go fires, even if other inputs read inside change','correct':True},
        {'t':'It returns a vector instead of a scalar'},
        {'t':'It runs on the client side'},
     ],
     'hints':['eventReactive = reactive that listens only to a specific event.','Like observeEvent, but produces a value you can read elsewhere.'],
     'why':'<code>eventReactive(eventExpr, valueExpr)</code> is the value-returning cousin of observeEvent: it caches a value that is updated only when eventExpr changes. Use it for "compute on click" semantics where downstream outputs still read a reactive value. Plain <code>reactive()</code> would re-compute every time any input it reads changes.'},

    # Easy code (2)
    {'id':'sh_code_app_basic', 'type':'code', 'diff':1, 'topic':'architecture',
     'prompt':'Write <code>build_simple_app()</code> that returns a shinyApp object whose UI has a numeric input with inputId <code>"n"</code> (default 5) and a text output with outputId <code>"squared"</code>, and whose server renders the square of input$n into output$squared.',
     'fnName':'build_simple_app',
     'starter':'library(shiny)\n\nbuild_simple_app <- function() {\n  # Your code here\n  \n}',
     'tests':'app <- build_simple_app()\nstopifnot(inherits(app, "shiny.appobj"))\nsrv_src <- if (is.function(app$serverFuncSource)) app$serverFuncSource() else app$server\nstopifnot(is.function(srv_src))\nbody_text <- paste(deparse(body(srv_src)), collapse = " ")\nstopifnot(grepl("input\\\\$n", body_text))\nstopifnot(grepl("output\\\\$squared", body_text))\nstopifnot(grepl("renderText", body_text))',
     ...
     'why':'<code>ui &lt;- fluidPage(numericInput("n", "n", 5), textOutput("squared")); server &lt;- function(input, output) { output$squared &lt;- renderText(input$n^2) }; shinyApp(ui, server)</code>. renderText reads input$n reactively and updates whenever the slider moves.'},
    {'id':'sh_code_input_ids', 'type':'code', 'diff':1, 'topic':'ui',
     'prompt':'Write <code>list_input_ids(ui)</code> that takes a shiny UI object (e.g. built with fluidPage()) and returns a character vector of inputId values found inside it. A simple HTML stringify with a regex over id attributes is acceptable.',
     'fnName':'list_input_ids',
     'starter':'library(shiny)\n\nlist_input_ids <- function(ui) {\n  # Your code here\n  \n}',
     'tests':'u <- fluidPage(\n  numericInput("a", "a", 1),\n  textInput("b", "b"),\n  selectInput("c", "c", choices = c("x","y"))\n)\nr <- list_input_ids(u)\nstopifnot(is.character(r))\nstopifnot(setequal(r, c("a","b","c")))',
     'hints':['html <- as.character(ui) stringifies the tag tree.','regmatches() + gregexpr() with pattern \'id="([^"]+)"\' captures all id attributes.','sub() the wrapper away and unique() to dedupe.'],
     'why':'<code>html &lt;- as.character(ui); ids &lt;- regmatches(html, gregexpr(\'id="[^"]+"\', html))[[1]]; unique(sub(\'id="([^"]+)"\', "\\\\1", ids))</code>. For a richer walk use htmltools tag inspection, but the stringify-and-regex trick is enough for fluidPage builds.'},

    # Medium code (2)
    {'id':'sh_code_summed_server', 'type':'code', 'diff':2, 'topic':'reactivity',
     'prompt':'Write <code>summed_server(input, output)</code>, a Shiny server function, that reads two numeric inputs <code>a</code> and <code>b</code> and renders their sum into output <code>total</code> using <code>renderText()</code>.',
     ...
     'hints':['function(input, output) { output$total <- renderText(input$a + input$b) }','renderText wraps a reactive expression that re-runs whenever input$a or input$b changes.'],
     'why':'<code>function(input, output) { output$total &lt;- renderText(input$a + input$b) }</code>. renderText returns a reactive observer that updates output$total whenever input$a or input$b changes.'},
    {'id':'sh_code_button_counter', 'type':'code', 'diff':2, 'topic':'reactivity',
     'prompt':'Write <code>button_server(input, output)</code>, a server function, where on each click of <code>input$go</code> (an action button) output <code>counter</code> shows the cumulative click count. Use <code>reactiveVal()</code> and <code>observeEvent()</code>.',
     ...
     'hints':['count <- reactiveVal(0) creates a mutable reactive integer initialized at 0.','observeEvent(input$go, count(count() + 1)) increments on each click.','output$counter <- renderText(count()) reads the value reactively.'],
     'why':'<code>count &lt;- reactiveVal(0); observeEvent(input$go, count(count() + 1)); output$counter &lt;- renderText(count())</code>. reactiveVal gives a mutable reactive cell; observeEvent fires only on clicks; renderText reads count() reactively and re-renders when it changes.'},

    # Hard code (2)
    {'id':'sh_code_module_ui', 'type':'code', 'diff':3, 'topic':'modules',
     'prompt':'Write a Shiny module UI function <code>counter_module_ui(id)</code> that, using <code>NS(id)</code>, returns a tagList with an actionButton (inputId "go", label "Go") and a textOutput (outputId "count").',
     ...
     'hints':['ns <- NS(id) creates a namespace function.','tagList(actionButton(ns("go"), "Go"), textOutput(ns("count"))).','The rendered HTML should show id="mymod-go" and id="mymod-count" because NS prefixes with the module id.'],
     'why':'<code>ns &lt;- NS(id); tagList(actionButton(ns("go"), "Go"), textOutput(ns("count")))</code>. <code>NS(id)</code> returns a function that prefixes ids with "id-" so multiple instances of the module can coexist in one app without id collisions.'},
    {'id':'sh_code_polling_server', 'type':'code', 'diff':3, 'topic':'reactivity',
     'prompt':'Write <code>polling_server(input, output)</code>, a server function, where output <code>now</code> displays <code>format(Sys.time())</code> and is re-rendered every 1000 ms using <code>invalidateLater()</code>.',
     'fnName':'polling_server',
     'starter':'library(shiny)\n\npolling_server <- function(input, output) {\n  # Your code here\n  \n}',
     'tests':'stopifnot(is.function(polling_server))\nbody_text <- paste(deparse(body(polling_server)), collapse = " ")\nstopifnot(grepl("invalidateLater", body_text))\nstopifnot(grepl("1000", body_text))\nstopifnot(grepl("output\\\\$now", body_text))\nstopifnot(grepl("Sys.time", body_text))',
     ...
     'why':'<code>output$now &lt;- renderText({ invalidateLater(1000); format(Sys.time()) })</code>. invalidateLater(1000) inside renderText schedules the reactive to re-fire after 1 second, producing a live clock without explicit polling. Pass the session argument for safety inside modules: <code>invalidateLater(1000, session)</code>.'},
]
