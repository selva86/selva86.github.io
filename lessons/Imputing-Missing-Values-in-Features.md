---
title: "Feature Engineering Lesson 6: Imputing Missing Values in Features"
catalog_blurb: "How to fill missing values without distorting results or leaking the test set."
description: "Impute missing features in R the leak-free way: median, group, kNN and model-based fills learned on the training set only, plus a missing-value flag, all inside the modeling pipeline."
keywords: "imputation, missing values, feature engineering, data leakage, median imputation, kNN imputation, model-based imputation, missing indicator, recipes, step_impute, train test split, R"
post_type: "LESSON"
curriculum_id: "6.60.6"
webr: true
mathjax: true
lesson_access: "pro"
course_id: "ds-feature-engineering"
course_title: "Feature Engineering in R"
course_lesson: "6"
course_total: "7"
course_landing: "R-Feature-Engineering-Course.html"
course_next: "Feature-Selection-and-Spotting-Leakage.html"
course_prev: "Features-from-Dates-Text-and-Geo.html"
---

=== step === cover
::eyebrow Lesson 6 of 7
## Imputing Missing Values in Features

You are building a model to predict the monthly rent of city apartments. Your scraper pulled 240 listings, each with the number of bedrooms, the floor, the rent, and the floor area in square feet. Floor area is the single strongest clue to rent. And about one listing in seven has it blank, because that landlord just never filled the box in.

Most models will not even start with a hole in the data. So you have to put a number in every gap. The catch, and the whole point of this lesson, is that the obvious ways to do it quietly cheat: they let your test set sneak into the answer, and they flatten the very signal you are trying to model. Here you will fill gaps the honest way.

By the end you will be able to:

- Impute a numeric feature with the median and keep a flag that remembers where the gap was
- Explain why imputation **leaks** unless the fill is learned on the training set alone, and do it leak-free
- Fill smarter by borrowing from similar rows (a group, the nearest neighbours, a small model)

**Prerequisites:** you can write a `mutate()` and a pipe ([The dplyr Verbs](The-dplyr-Verbs.html)), you know what a train/test split and data leakage are ([Train, Validation, Test, and Data Leakage](Train-Validation-Test-and-Data-Leakage.html)), and you have met `NA` and the three missingness mechanisms ([Missing Value Treatment](Missing-Value-Treatment.html)). The new ideas are taught from scratch.

Press Run on the table below: each blank floor area is filled with the column median, the simplest possible stand-in.

::widget table-transform {"caption":"Each missing floor area filled with the column median, the plainest stand-in. The green area_filled column keeps every listing instead of throwing the blank ones away.","before":{"cols":["id","bedrooms","area_sqft","rent"],"rows":[[101,1,540,1850],[102,2,null,2600],[103,3,1180,3500],[104,2,880,2550],[105,1,null,1700],[106,4,1520,4200]]},"after":{"cols":["id","bedrooms","area_sqft","rent","area_filled"],"rows":[[101,1,540,1850,540],[102,2,null,2600,1030],[103,3,1180,3500,1180],[104,2,880,2550,880],[105,1,null,1700,1030],[106,4,1520,4200,1520]]},"code":"library(dplyr)\n# Blanks scrape in as \"\" - make the gaps explicit, then fill with the median.\ndf %>%\n  mutate(area_sqft = as.numeric(na_if(area_sqft, \"\")),\n         area_filled = ifelse(is.na(area_sqft),\n                              median(area_sqft, na.rm = TRUE), area_sqft))"}

=== step === concept
::eyebrow Why you cannot skip it
## A gap is not a number, so the model stops

Hand a linear regression, a glmnet, or a kNN a column with a single `NA` in it and most of them refuse to run, or silently drop every row that has one. With about one in seven listings missing its area, dropping those rows would throw away a seventh of your data before training even begins, and in the [last lesson on missing data](Missing-Value-Treatment.html) you saw the worse problem: the rows that go missing are rarely a fair sample, so deleting them bends the answer.

So the move in a modeling pipeline is almost always to **impute**: put a plausible number in each gap and keep the row. Here is the shift in mindset that makes this a feature-engineering lesson and not just data cleaning. A filled value is not a recovered measurement; it is a **new feature you are manufacturing**, a deliberate guess. And like any feature, it can be built carelessly (leaking, distorting) or built well.

First, the data. Each lesson runs in a fresh R session, so we build the 240 listings right here (run this once). The area depends on the number of bedrooms, with some noise, exactly the kind of structure a smart fill can exploit later.

```r
library(dplyr)

set.seed(42)
n <- 240
flats <- data.frame(
  bedrooms = sample(1:4, n, replace = TRUE, prob = c(.30, .40, .20, .10)),
  floor    = sample(1:18, n, replace = TRUE)
)
flats$area_sqft <- round(320 * flats$bedrooms + rnorm(n, 0, 110) + 180)
flats$rent      <- round(2.1 * flats$area_sqft + 240 * flats$bedrooms + rnorm(n, 0, 250))

# Knock a blank into 36 of the 240 floor areas (the landlord left the box empty).
set.seed(7)
flats$area_sqft[sample(n, 36)] <- NA

colSums(is.na(flats))
#> bedrooms    floor area_sqft     rent
#>        0        0       36        0
```

Thirty-six blanks, all in `area_sqft`, one listing in seven. Now the question that drives the rest of the lesson: what number do we put in each one?

=== step === concept
::eyebrow The plain fix, and its cost
## Fill with the median, and remember the gap

The plainest fill is a single central value for the whole column. Replace each missing area with \(m = \text{median}(x_{\text{obs}})\), where \(x_{\text{obs}}\) are the areas that **were** recorded and \(m\) is their middle value. We reach for the median rather than the mean because floor area is right-skewed (a few large penthouses), and the median shrugs those off while the mean gets dragged upward.

But filling many gaps with one identical number does something you must own. Every imputed listing now sits exactly at \(m\), so the column's natural spread shrinks. Watch it happen on the real data:

```r
filled <- ifelse(is.na(flats$area_sqft),
                 median(flats$area_sqft, na.rm = TRUE),
                 flats$area_sqft)

c(sd_observed = round(sd(flats$area_sqft, na.rm = TRUE)),
  sd_filled   = round(sd(filled)))
#> sd_observed   sd_filled
#>         344         317
```

The standard deviation (the typical distance of a value from the average, our measure of spread) drops once the gaps collapse onto one point. The fill also weakens the real area-to-rent correlation, because 36 listings now carry an area that ignores everything else about them.

[KEY INSIGHT]
A constant fill keeps every row but flattens the column: less spread, weaker correlations. That is the price of the simplest imputation, and it is exactly why we keep a record of which values were guessed, and why the smarter fills later in this lesson exist.

That record is a second engineered feature: a **missing-value indicator**, a 0/1 column that is 1 wherever the area was originally blank. It costs nothing and it lets the model learn whether a missing area is itself a signal (maybe blank-area listings are systematically older, or cheaper). You will build both the fill and the flag next.

=== step === tryit
::eyebrow Your turn
## Impute the area, keep the flag

Fill every missing `area_sqft` with the column median, and in the same step add `area_was_missing`, a 0/1 flag that records where the gap was. The flag is computed **first**, from the original column, before the fill erases the evidence. You write the median stand-in that goes into `ifelse()`; remember `na.rm = TRUE` so the median ignores the blanks themselves.

```r
flats_imp <- flats %>%
  mutate(
    area_was_missing = as.integer(is.na(area_sqft)),
    area_sqft        = ifelse(is.na(area_sqft), ____, area_sqft)
  )
sum(flats_imp$area_was_missing)   # how many we filled
```
::check {"regex":"median\\s*\\(\\s*area_sqft\\s*,\\s*na\\.rm\\s*=\\s*TRUE","gate":true,"difficulty":"beginner","ok":"That is the mean's sturdier cousin: median(area_sqft, na.rm = TRUE) is computed from the recorded areas and dropped into every gap, while area_was_missing quietly remembers which 36 rows were guessed.","no":"Inside ifelse() put the median of the recorded areas: median(area_sqft, na.rm = TRUE). The na.rm = TRUE tells it to ignore the blanks when computing that middle value."}
::solution
```r
flats_imp <- flats %>%
  mutate(
    area_was_missing = as.integer(is.na(area_sqft)),
    area_sqft        = ifelse(is.na(area_sqft),
                              median(area_sqft, na.rm = TRUE), area_sqft)
  )
sum(flats_imp$area_was_missing)
#> [1] 36
```

=== step === quiz
::eyebrow Check yourself
## Mean or median?

Floor area in your listings is right-skewed: most flats are small to mid-sized, but a handful of luxury units run very large. You are about to fill the blanks with one central value. Why is the **median** the safer default than the **mean** here?

::quiz {"correct":2,"gate":true,"difficulty":"intermediate"}
- The mean cannot be computed when a column has any NA values. ::no Not the issue: mean(area_sqft, na.rm = TRUE) computes fine even with gaps, exactly as the median does. The real reason to prefer the median here is that on right-skewed data its central value is more representative, while the mean gets pulled toward the few very large flats.
- The few very large flats drag the mean upward, so it overstates a typical area; the median sits at the middle and is barely moved by those outliers. ::ok Right. On right-skewed data the mean is pulled toward the long tail, so mean-imputing makes every blank flat look larger than a typical one. The median stays at the centre of the data and gives a sturdier stand-in.
- The median changes the column's spread less than the mean does. ::no Both replace the gaps with a single constant, so both shrink the spread by the same amount. The reason to prefer the median is that its central value is more representative on skewed data, not that it preserves spread.

=== step === concept
::eyebrow The trap that defines this lesson
## Imputation learns something, so it can leak

Look again at what you just did: you computed `median(area_sqft, na.rm = TRUE)` over the data and used it to fill the gaps. That median is a thing **learned from the rows**. And the iron rule of honest modeling is that anything learned must be learned from the **training set only**, never from the test set.

If you compute the median over the *whole* dataset, including the test listings, and then split, the fill value has already peeked at the test rows. Your test score gets a small, invisible boost it would never get on genuinely new listings, where no test median exists. That is **leakage**: information from outside the training fold sneaking into the model. Drag the toggle below to feel how a leaked feature inflates a test score to something too good to be true.

The fix is a discipline, not a function. Split first. Then learn the fill on the training rows alone:

\[ m_{\text{train}} = \text{median}(x_{\text{train, obs}}) \]

where \(x_{\text{train, obs}}\) are the recorded areas **in the training set**. Store that one number, and use the same \(m_{\text{train}}\) to fill the gaps in the training set and in the test set. The test rows never get a vote in what the fill is; they only receive it.

::widget data-split {}

=== step === tryit
::eyebrow Your turn
## Fill it leak-free

Split the listings, then learn the fill on the training rows only. Complete the line that computes `fill_value`: it must take the median of the **training** areas, not the whole dataset. Then the same number is applied to both sets.

```r
set.seed(1)
idx   <- sample(nrow(flats), 0.7 * nrow(flats))
train <- flats[idx, ]
test  <- flats[-idx, ]

# Learn the fill on the TRAINING rows alone:
fill_value <- median(____$area_sqft, na.rm = TRUE)

# Apply that one stored number to BOTH sets:
train$area_sqft[is.na(train$area_sqft)] <- fill_value
test$area_sqft[is.na(test$area_sqft)]   <- fill_value
fill_value
```
::check {"regex":"median\\s*\\(\\s*train\\$area_sqft","gate":true,"difficulty":"intermediate","ok":"Exactly right: the fill is median(train$area_sqft, na.rm = TRUE), learned on training rows only. The test gaps receive that stored number but never influence it, so the test set stays an honest stand-in for the future.","no":"Take the median of the TRAINING areas: median(train$area_sqft, na.rm = TRUE). Using flats (the whole dataset) would let the test rows shape the fill, which is the leak you are trying to avoid."}
::solution
```r
set.seed(1)
idx   <- sample(nrow(flats), 0.7 * nrow(flats))
train <- flats[idx, ]
test  <- flats[-idx, ]

fill_value <- median(train$area_sqft, na.rm = TRUE)

train$area_sqft[is.na(train$area_sqft)] <- fill_value
test$area_sqft[is.na(test$area_sqft)]   <- fill_value
fill_value
#> [1] 851
```

=== step === concept
::eyebrow Fill smarter
## Borrow from similar rows

A single median ignores everything else you know about a flat. A blank 1-bedroom studio and a blank 4-bedroom both get the same 851 sq ft, which is too big for one and too small for the other. The smarter fills use the **other columns** to make a tailored guess.

The simplest version groups by a key feature and fills from that group. Fill a blank with the median area of flats with the **same number of bedrooms**, and a studio gets a studio-sized guess. Press Run:

::widget table-transform {"caption":"The same two gaps, now filled with the median area of flats that have the SAME number of bedrooms. The 1-bed gets a small area, the 2-bed a larger one, instead of one flat 1030 for both.","before":{"cols":["id","bedrooms","area_sqft","rent"],"rows":[[101,1,540,1850],[102,2,null,2600],[103,3,1180,3500],[104,2,880,2550],[105,1,null,1700],[106,4,1520,4200]]},"after":{"cols":["id","bedrooms","area_sqft","rent","area_smart"],"rows":[[101,1,540,1850,540],[102,2,null,2600,880],[103,3,1180,3500,1180],[104,2,880,2550,880],[105,1,null,1700,540],[106,4,1520,4200,1520]]},"code":"library(dplyr)\ndf %>%\n  mutate(area_sqft = as.numeric(na_if(area_sqft, \"\"))) %>%\n  group_by(bedrooms) %>%\n  mutate(area_smart = ifelse(is.na(area_sqft),\n                            median(area_sqft, na.rm = TRUE), area_sqft)) %>%\n  ungroup()"}

**k-nearest-neighbours (kNN) imputation** generalises that idea past a single grouping column. For a flat with a blank area, it finds the \(k\) recorded flats most similar across the *other* features (bedrooms, floor, rent) and averages their areas:

\[ \hat{x}_i = \frac{1}{k} \sum_{j \in N_k(i)} x_j \]

where \(N_k(i)\) is the set of the \(k\) nearest recorded flats to flat \(i\) (nearest by distance over the other columns) and \(x_j\) is the recorded area of neighbour \(j\).

**Model-based imputation** goes one step further and fits an actual model to predict the missing column from the others, learned on the rows that have it, then predicts the gaps:

\[ \hat{x}_i = \hat\beta_0 + \hat\beta_1\,\text{bedrooms}_i + \hat\beta_2\,\text{floor}_i \]

```r
set.seed(1)
idx   <- sample(nrow(flats), 0.7 * nrow(flats))
train <- flats[idx, ]
have  <- train[!is.na(train$area_sqft), ]   # training flats that HAVE an area
miss  <- which(is.na(train$area_sqft))       # the gaps to fill

# Learn area from the other columns, on the recorded rows only:
area_model <- lm(area_sqft ~ bedrooms + floor, data = have)
train$area_sqft[miss] <- round(predict(area_model, train[miss, ]))

round(coef(area_model), 1)
#> (Intercept)    bedrooms       floor
#>       148.1       332.3         0.0
```

The bedrooms coefficient, about 332, is close to the 320 we built into the data: each extra bedroom adds roughly 320 sq ft, while floor barely moves area (its coefficient is essentially zero, exactly as it should be). A model-based fill hands each gap a value that honours that relationship instead of one flat number.

[NOTE]
Two cautions. First, these smarter fills still **learn from the rows**, so the same leak rule applies: fit the group medians, the neighbours, or the regression on the training fold only (a recipe's `step_impute_knn()` or `step_impute_linear()` does this for you). Second, filling with a single best guess pretends you are certain about a value you are not. When that uncertainty matters for inference, **multiple imputation** (the `mice` package) fills each gap several times to carry the doubt forward, rather than hiding it.

=== step === quiz
::eyebrow Check yourself
## Spot the leak

A teammate reports a great cross-validation score on the rent model. Their pipeline does this, in order: (1) fill every missing `area_sqft` with the median area of the **full** dataset, (2) split into folds, (3) cross-validate. You are suspicious. What is wrong, and what would fix it?

::quiz {"correct":1,"gate":true,"difficulty":"advanced"}
- The median was computed over all rows before splitting, so every fold's validation rows helped set the fill they are later scored on: leakage. Move the imputation inside the fold (learn the median on each fold's training part only). ::ok Exactly. Imputing before the split lets validation data influence the fill, so the score is optimistic. The fix is to make imputation part of the resampled pipeline (a recipe inside the fold), so each fill is learned on training rows alone.
- Nothing is wrong: the median is a single number, so it cannot carry enough information to leak. ::no The statistic is not the problem; the timing is. A mean computed over the full dataset before splitting leaks in exactly the same way. Any fill learned before the split, mean, median, kNN or model, sees the validation rows.
- The mistake is using the median at all; a mean computed the same way would not leak. ::no The statistic is not the problem; the timing is. A mean computed over the full dataset before splitting leaks in exactly the same way. Any fill learned before the split, mean, median, kNN or model, sees the validation rows.

=== step === concept
::eyebrow Go deeper
## References

A few authoritative places to take this further:

- [Kuhn and Johnson, Feature Engineering and Selection (free online)](https://www.feat.engineering/) - the chapter on missing data and imputation, written for the modeling pipeline you are building here.
- [van Buuren, Flexible Imputation of Missing Data (free online)](https://stefvanbuuren.name/fimd/) - the standard modern text; explains why a single imputed value understates uncertainty and how multiple imputation (`mice`) fixes it.
- [tidymodels recipes: step_impute_knn() reference](https://recipes.tidymodels.org/reference/step_impute_knn.html) - the leak-free way to impute inside a pipeline, with `step_impute_median`, `step_impute_linear` and friends alongside it.
- [scikit-learn user guide: Common pitfalls and recommended practices](https://scikit-learn.org/stable/common_pitfalls.html) - the clearest short write-up of why fitting any preprocessing (including imputation) on all the data leaks, with the train-only fix.

=== step === complete
## Lesson 6 complete

You can now treat a hole in a feature as something you engineer with care. You filled gaps with the median and kept an `area_was_missing` flag so the model still knows where the guesses are; you saw a constant fill shrink the column's spread, the price of the simplest method. Most importantly, you learned the rule that runs through this whole section: imputation **learns** a value, so it must learn it on the training set alone, or it leaks. And you went beyond one flat number to fills that borrow from similar rows, by group, by nearest neighbours, by a small model, each handing a blank a value that respects everything else you know about that flat.

Next, Lesson 7: Feature Selection and Spotting Leakage. You have now seen leakage sneak in through an innocent-looking fill. In the finale you will learn to choose the features that actually earn their place, and to catch the more dangerous leaks, the ones hiding inside a feature itself, before they flatter a score you cannot trust.
