...
Part 1 left one thing sitting on the table. Ravi's Hillside orders scattered about five times as widely as Lakeview's, and all the advice you got was one line: use the Welch version. Today we find out what that line was covering up.
...
**What you need first:** you can read a simple R script, so a variable, `c()`, a function call and `$` are familiar. No statistics is assumed. Standard deviation, standard error, degrees of freedom, false-alarm rate and every other term get defined in plain words the first time they turn up. Part 1 helps, and it is not required.
